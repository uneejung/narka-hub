    ...
    <div>
      ...
      {prod ? (
        <div className="space-y-4">
          {prod.usps.map(u => (
            <div key={u.id} className="bg-white/80 border border-gray-100 rounded-xl p-4 shadow-sm backdrop-blur-sm">
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        <div className="bg-white/80 border border-gray-100 rounded-xl p-4 text-center shadow-sm"><p className="text-2xl font-black">{prod.usps.length}</p><p className="text-xs text-gray-400 mt-0.5">전체 편익 꼭지</p></div>
        <div className="bg-white/80 border border-indigo-100 rounded-xl p-4 text-center shadow-sm"><p className="text-2xl font-black text-indigo-600">{coveredUsps}</p><p className="text-xs text-gray-400 mt-0.5">소재 있는 꼭지</p></div>
        <div className="bg-white/80 border border-gray-100 rounded-xl p-4 text-center shadow-sm"><p className="text-2xl font-black">{totalAssets}</p><p className="text-xs text-gray-400 mt-0.5">이 품목 소재 수</p></div>
      </div>
      {prod.usps.length === 0 ? (
...
function RawDataTable({ filtered }) {
  ...
  return (
    <div className="bg-white/80 border border-gray-100 rounded-xl p-4 shadow-sm backdrop-blur-sm">
...
function MetaRawTab({ csvRows, setCsvRows, assets, setAssets, products }) {
  ...
  return (
    <div>
      <div className="bg-white/80 border border-gray-100 rounded-xl p-5 shadow-sm mb-5 backdrop-blur-sm">
            ...
            return (
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-5">
                {summaryCards.map(([l, v, c]) => (
                  <div key={l} className="bg-white/80 border border-gray-100 rounded-xl p-4 shadow-sm backdrop-blur-sm">
...
function AppInner({ user, signOut }) {
  ...
  return (
    <div className="min-h-screen font-sans" style={{ background: "#F8F3DA" }}>
      <div className="sticky top-0 z-40 shadow-sm" style={{ background: "#FFEEA2" }}>
        <div className="max-w-7xl mx-auto px-4 py-2.5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src="/Narka-Wordmark-Black.png" alt="NARKA" className="h-5 object-contain" />
